...
export default function SalesPage() {
  ...
  return (
    <div className="space-y-5">
      ...
      <Card title="Add Sale"><form onSubmit={submit} className="space-y-4"><div className="theme-form-grid">{["product_id","sale_date","quantity","revenue"].map((n) => <div className="theme-field" key={n}><label>{n.replace("_"," ")}</label><input name={n} type={n === "sale_date" ? "date" : "text"} placeholder={n.replace("_"," ")} className="form-control" required /></div>)}</div><div className="border-t border-line pt-4"><LoadingButton loading={saving} type="submit">Save Sale</LoadingButton></div></form></Card>
        <Card title="Upload Sales CSV"><div className="mb-3 flex items-center gap-2 text-sm font-bold text-muted"><Upload className="h-4 w-4" />Bulk import historical sales</div><UploadCsv onUpload={async (form) => { if (!window.confirm("Are you sure you want to process this file?")) return false; await salesService.upload(form); await load(); }} /></Card>
