...
export default function ReviewsPage() {
  ...
  return (
    <div className="space-y-5">
      ...
      <Card title="Add New Review"><form onSubmit={submit} className="space-y-4"><div className="theme-form-grid"><div className="theme-field"><label>Product ID</label><input name="product_id" placeholder="product id" className="form-control" required /></div><div className="theme-field"><label>Rating</label><input name="rating" placeholder="rating 1-5" className="form-control" required /></div><div className="theme-field md:col-span-2 xl:col-span-1"><label>Review text</label><textarea name="review_text" placeholder="Customer feedback..." className="form-control min-h-24" required /></div></div><div className="border-t border-line pt-4"><LoadingButton loading={saving} type="submit"><span className="inline-flex items-center gap-2"><MessageSquarePlus className="h-4 w-4" />Analyze & Save</span></LoadingButton></div></form></Card>
        <Card title="Upload Reviews CSV"><div className="mb-3 flex items-center gap-2 text-sm font-bold text-muted"><Upload className="h-4 w-4" />Bulk import review feedback</div><UploadCsv onUpload={async (form) => { if (!window.confirm("Are you sure you want to process this file?")) return false; await reviewService.upload(form); load(); }} /></Card>
