...
export function DataTable<T extends { id?: number | string }>({ columns, rows, loading = false }: { columns: Column<T>[]; rows: T[]; loading?: boolean }) {
  ...
  if (!rows?.length) return <div className="rounded-md border border-dashed border-slate-300 p-6 text-sm text-slate-500">No data available.</div>;
  return (
    <div className="overflow-x-auto">
      <table className="w-full min-w-[720px] text-left text-sm">
        <thead>
          <tr className="border-b border-slate-200 text-xs uppercase text-slate-500">
            {columns.map((c) => <th key={String(c.key)} className="px-3 py-3 font-semibold">{c.label}</th>)}
          </tr>
        </thead>
        <tbody>
          {rows.map((row, index) => (
